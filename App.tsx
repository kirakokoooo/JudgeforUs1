...
import { SketchBox, SketchButton, SketchInput, StickFigureAvatar, FloatingCard, ChatBubble, ShoutingAnimation, AvatarMood, AvatarVariant } from './components/SketchComponents';

const MAX_ROUNDS = 3;

// Helper for random avatars
const AVATAR_VARIANTS: AvatarVariant[] = ['cat', 'bear', 'rabbit', 'dog', 'pig', 'human'];
const AVATAR_COLORS = ['#ef4444', '#f97316', '#84cc16', '#06b6d4', '#8b5cf6', '#ec4899', '#6366f1'];

const getRandomVariant = (seed: number) => AVATAR_VARIANTS[seed % AVATAR_VARIANTS.length];
const getRandomColor = (seed: number) => AVATAR_COLORS[seed % AVATAR_COLORS.length];
...
const App = () => {
  ...
  const handleDraftComplete = (playerId: 1 | 2) => {
    ...
    // Rule Change: Must select at least 3 arguments to ensure 3 rounds of play
    if (player.team.length < 3) {
      alert("请至少选择 3 个观点，才能吵满 3 回合哦！");
      return;
    }
  ...
  const playDialogueAnimation = async (dialogue: any[]) => {
    ...
    for (const line of dialogue) {
      await new Promise(r => setTimeout(r, 1200)); // Slightly faster for flow
  ...
  const handleNextRoundClick = () => {
    // Explicitly use current + 1 to move to the next logical round
    const nextRoundIdx = gameState.currentRoundIndex + 1;
  ...
  useEffect(() => {
    ...
    const isRoundPlayed = gameState.rounds.some(r => r.roundNumber === roundIndex + 1);
          ...
          <div className="flex flex-wrap justify-center content-start gap-2 md:gap-4 h-full overflow-y-auto scrollbar-hide py-2 md:py-4 pb-20">
            {generatedArgs.map((arg, idx) => (
              <FloatingCard 
                ...
                avatarVariant={getRandomVariant(idx)}
                avatarColor={getRandomColor(idx + (isP1 ? 0 : 5))}
  ...
  if (gameState.phase === AppPhase.CELEBRATION) {
     return (
       <div className="min-h-screen p-4 flex flex-col items-center justify-center max-w-2xl mx-auto overflow-hidden relative">
          {/* Confetti-like background elements */}
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
             {[...Array(20)].map((_, i) => (
                <div key={i} className="absolute animate-float text-4xl" style={{ 
